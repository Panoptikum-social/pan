defmodule Pan.Search do
  ...
  def batch_index(%{model: model, preloads: preloads, selects: selects}) do
    record_ids =
    ...
    manticore_data =
      ...
      |> Repo.all()
      |> Enum.map(&model.manticore_struct(&1))
    ...
    if response_code == 200 && length(record_ids) > 0 do
      batch_index(%{model: model, preloads: preloads, selects: selects})
    end
  end
end
