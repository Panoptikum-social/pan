defmodule Pan.Tools.FixtureGenerator do
  import Ecto.Query
  alias Pan.Repo
  alias PanWeb.{AlternateFeed, CategoryPodcast, Chapter, Delegation, Enclosure,
                Engagement, Episode, Feed, FeedBacklog, Follow,
                Gig, Invoice, Like, Manifestation, Message,
                Opml, Persona, Podcast, Recommendation, Subscription,
                User}
  ...
  def drill_down do
    ...
    episode_ids = (from e in Episode, where: e.podcast_id > 250,
                                      select: e.id)
                  |> Repo.all()
    ...
    feed_ids = (from f in Feed, where: f.podcast_id > 250,
                                select: f.id)
               |> Repo.all()
    ...
    ids_with_gigs = from(g in Gig, group_by: g.persona_id,
                                   select:   g.persona_id)
                     |> Repo.all()

    ids_with_engagements = from(e in Engagement, group_by: e.persona_id,
                                                 select:   e.persona_id)
                           |> Repo.all()
