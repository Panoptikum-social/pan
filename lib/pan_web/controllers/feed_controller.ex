defmodule PanWeb.FeedController do
  ...
  def datatable(conn, params) do
    ...
    query =
      if search != "" do
        from(f in Feed,
          ...
          where:
            ilike(f.self_link_title, ^searchfrag) or
    ...
    feeds =
      case order_by do
        [asc: :podcast_episodes_count] ->
          from([_, podcast] in feeds, order_by: [asc: podcast.episodes_count])

        [desc: :podcast_episodes_count] ->
          from([_, podcast] in feeds, order_by: [desc: podcast.episodes_count])

        _ ->
          from([_, _] in feeds, order_by: ^order_by)
      end
      |> Repo.all()
  ...
  def update(conn, %{"id" => id, "feed" => feed_params}) do
    ...
    feed_params =
      if feed_params["alternate_feeds"]["0"]["title"] == nil and
           feed_params["alternate_feeds"]["0"]["url"] == nil do
