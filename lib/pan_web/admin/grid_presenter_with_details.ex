defmodule PanWeb.Admin.GridPresenterWithDetails do
  ...
  def present(presenter, record, _field, _format) when is_function(presenter),
    do: presenter.(record)
  ...
  def render(assigns) do
    ~H"""
    <div
      :if={@model.__schema__(:redact_fields) |> Enum.member?(@field) |> Kernel.not()}
      class={[
    ...
    <div
      :if={@model.__schema__(:redact_fields) |> Enum.member?(@field)}
      class={[
