defmodule PanWeb.Live.Moderation.EditEpisode do
  ...
  def mount(%{"id" => category_id, "episode_id" => episode_id}, session, socket) do
    ...
    columns = [
      :id, :podcast_id, :title, :link, :publishing_date, :guid, :description, :shownotes, :payment_link_title,
      :payment_link_url, :deep_link, :duration, :subtitle, :summary, :image_title, :image_url,
      :full_text, :inserted_at, :updated_at
    ]
  ...
  def handle_info({:saved, %{message: message}}, socket) do
    episode_grid_path = Routes.moderation_frontend_path(socket, :episode_grid, socket.assigns.category_id, socket.assigns.episode.podcast_id)
