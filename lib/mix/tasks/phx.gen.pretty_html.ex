defmodule Mix.Tasks.Phx.Gen.PrettyHtml do
  ...
  def inputs(%Schema{} = schema) do
    Enum.map(schema.attrs, fn
      ...
      {key, :decimal} ->
        {label(key), ~s(<%= number_input f, #{inspect(key)}, step: "any" %>), error(key)}
      {key, :boolean} ->
        {checkbox_label(key), ~s(<%= checkbox f, #{inspect(key)}, class: "form-checkbox" %>), error(key)}
      {key, :text} ->
        {label(key), ~s(<%= textarea f, #{inspect(key)}, class: "form-input" %>), error(key)}
      {key, :date} ->
      ...
      {key, _}  ->
        {label(key), ~s(<%= text_input f, #{inspect(key)}, class: "form-input" %>), error(key)}
    end)
  end
  ...
  defp checkbox_label(key) do
    ~s(<%= label f, #{inspect(key)}, class: "form-label ml-1 inline" %>)
  end
