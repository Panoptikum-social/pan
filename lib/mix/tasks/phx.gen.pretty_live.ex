defmodule Mix.Tasks.Phx.Gen.PrettyLive do
  ...
  Alternatively, the `--context-app` option may be supplied to the generator:
      ...
      mix phx.gen.pretty_live Sales User users --context-app warehouse
  ...
  module name, for example:
      ...
      mix phx.gen.pretty_live Sales User users --web Sales
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise "mix phx.gen.pretty_live can only be run inside an application directory"
    end

    {context, schema} = Gen.Context.build(args)
    Gen.Context.prompt_for_code_injection(context)

    binding = [context: context, schema: schema, inputs: Gen.PrettyHtml.inputs(schema)]
  ...
  defp copy_new_files(%Context{} = context, binding, paths) do
    ...
    Mix.Phoenix.copy_from(paths, "priv/templates/phx.gen.pretty_live", binding, files)
