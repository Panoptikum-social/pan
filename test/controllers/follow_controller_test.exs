defmodule Pan.FollowControllerTest do
  ...
  describe "when user is logged in and is an admin" do
    ...
    test "creates resource and redirects when data is valid", %{conn: conn} do
      ...
      conn =
        post(conn, follow_path(conn, :create),
          follow: %{follower_id: follower.id, podcast_id: podcast.id}
        )
    ...
    test "updates chosen resource and redirects when data is valid", %{conn: conn} do
      ...
      conn =
        put(conn, follow_path(conn, :update, follow),
          follow: %{follower_id: follower.id, podcast_id: podcast.id}
        )

      assert redirected_to(conn) == follow_path(conn, :show, follow)
      assert Repo.get_by(Follow, %{follower_id: follower.id, podcast_id: podcast.id})
    end
