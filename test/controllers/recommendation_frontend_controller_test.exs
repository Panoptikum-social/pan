defmodule Pan.RecommendationFrontendControllerTest do
  ...
  test "lists all entries on index", %{conn: conn} do
    ...
    recommendation = insert_recommendation(%{user_id: user.id,
  ...
  test "lists my_recommendations", %{conn: conn} do
    ...
    recommendation = insert_recommendation(%{user_id: user.id,
                                             podcast_id: podcast.id})
    another_user = insert_user(%{name: "Another User",
                                 username: "auser" ,
  ...
  test "can create a podcast recommendation and redirects back", %{conn: conn} do
    ...
    conn = assign(conn, :current_user, user)
           ...
           |> post(recommendation_frontend_path(conn, :create,
                                                %{recommendation: %{podcast_id: podcast.id,
  ...
  test "can create an episode recommendation and redirects back", %{conn: conn} do
    ...
    conn = assign(conn, :current_user, user)
           ...
           |> post(recommendation_frontend_path(conn, :create,
                                                %{recommendation: %{episode_id: episode.id,
                                                                    comment: "recommendation comment"}}))
  ...
  test "can create an chapter recommendation and redirects back", %{conn: conn} do
    ...
    conn = assign(conn, :current_user, user)
           |> Plug.Conn.put_req_header("referer", "/episodes/" <> Integer.to_string(episode.id))
           |> post(recommendation_frontend_path(conn, :create,
                                                %{recommendation: %{chapter_id: chapter.id,
                                                                    comment: "recommendation comment"}}))
