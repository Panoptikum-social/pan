defmodule Pan.DelegationControllerTest do
  ...
  describe "when user is logged in and is an admin" do
    ...
    test "creates resource and redirects when data is valid", %{conn: conn} do
      ...
      delegate =
        insert_persona(%{pid: "delegate pid", name: "delegate name", uri: "delegate uri"})

      conn =
        post(conn, delegation_path(conn, :create),
          delegation: %{persona_id: persona.id, delegate_id: delegate.id}
        )

      assert redirected_to(conn) == delegation_path(conn, :index)
      assert Repo.get_by(Delegation, %{persona_id: persona.id, delegate_id: delegate.id})
    end
    ...
    test "updates chosen resource and redirects when data is valid", %{conn: conn} do
      ...
      conn =
        put(conn, delegation_path(conn, :update, delegation),
          delegation: %{persona_id: persona.id, delegate_id: delegate.id}
        )
