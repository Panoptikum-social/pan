defmodule Pan.SubscriptionControllerTest do
  ...
  describe "when user is logged in and is an admin" do
    ...
    test "creates resource and redirects when data is valid", %{conn: conn} do
      ...
      conn =
        post(conn, subscription_path(conn, :create),
          subscription: %{user_id: user.id, podcast_id: podcast.id}
        )

      assert redirected_to(conn) == subscription_path(conn, :index)
      assert Repo.get_by(Subscription, %{user_id: user.id, podcast_id: podcast.id})
    end
    ...
    test "updates chosen resource and redirects when data is valid", %{conn: conn} do
      ...
      conn =
        put(conn, subscription_path(conn, :update, subscription),
          subscription: %{user_id: user.id, podcast_id: podcast.id}
        )
    ...
    test "does not update chosen resource and renders errors when data is invalid", %{conn: conn} do
      ...
      conn =
        put(conn, subscription_path(conn, :update, subscription), subscription: @invalid_attrs)
